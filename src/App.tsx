...
const Floor3D = React.memo(function Floor3D({ data, onImpact, onLanded, currentProgress = 0, isCurrent = false }: {
  ...
  useEffect(() => {
    ...
    if (data.id === 0 && data.stage === 'FLOOR') {
      setHasImpacted(true);
      onLanded();
    }
  }, [data.isLanded, data.id, data.stage, onLanded]);
...
export default function App() {
  ...
  const [resources, setResources] = useState({ bricks: 10, bitumen: 5, wood: 5 });
  const [siteResources, setSiteResources] = useState({ bricks: 0, bitumen: 0, wood: 0 });
  ...
  // Use refs for the simulation loop to avoid interval resets and stale state
  const resourcesRef = useRef(resources);
  const siteResourcesRef = useRef(siteResources);
  const workersRef = useRef(workers);
  const floorsRef = useRef(floors);
  const currentFloorIndexRef = useRef(currentFloorIndex);
  const progressRef = useRef(progress);

  useEffect(() => { resourcesRef.current = resources; }, [resources]);
  useEffect(() => { siteResourcesRef.current = siteResources; }, [siteResources]);
  useEffect(() => { workersRef.current = workers; }, [workers]);
  useEffect(() => { floorsRef.current = floors; }, [floors]);
  useEffect(() => { currentFloorIndexRef.current = currentFloorIndex; }, [currentFloorIndex]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const sr = siteResourcesRef.current;
      const p = progressRef.current;

      let triggerBuild: BuildStage | null = null;
      let nextResources = { ...r };
      let nextSiteResources = { ...sr };
      let nextProgress = { ...p };

      // 1. Production
      if (currentWorkers.brickmaking > 0) {
        nextProgress.brickmaking += currentWorkers.brickmaking * BASE_SPEED;
        if (nextProgress.brickmaking >= 100) {
          nextResources.bricks += 1;
          nextProgress.brickmaking = 0;
        }
      }
      if (currentWorkers.bitumen > 0) {
        nextProgress.bitumen += currentWorkers.bitumen * BASE_SPEED * 0.7;
        if (nextProgress.bitumen >= 100) {
          nextResources.bitumen += 1;
          nextProgress.bitumen = 0;
        }
      }
      if (currentWorkers.wood > 0) {
        nextProgress.wood += currentWorkers.wood * BASE_SPEED * 0.5;
        if (nextProgress.wood >= 100) {
          nextResources.wood += 1;
          nextProgress.wood = 0;
        }
      }

      // 2. Transport - Pick resource that is most needed at site or just existing
      const transportable = (['bricks', 'bitumen', 'wood'] as const).filter(r => nextResources[r] > 0);
      if (currentWorkers.transport > 0 && transportable.length > 0) {
        nextProgress.transport += currentWorkers.transport * BASE_SPEED * 0.8;
        if (nextProgress.transport >= 100) {
          // Sort by what we have LEAST of at the site to keep stock balanced
          const toMove = transportable.sort((a, b) => nextSiteResources[a] - nextSiteResources[b])[0];

          if (toMove) {
            nextResources[toMove] -= 1;
            nextSiteResources[toMove] += 1;
          }
      ...
      // 3. Construction
      const currentFloor = currentFloors[floorIdx];
      const canBuildNext = currentFloor && (currentFloor.stage === 'NONE' || currentFloor.isLanded);

      if (canBuildNext && currentWorkers.construction > 0) {
        ...
        if (nextStage !== 'NONE') {
          ...
          const hasEnough = Object.entries(costs).every(([res, amount]) =>
            nextSiteResources[res as keyof typeof siteResources] >= (amount as number) * 0.05
          );

          if (hasEnough) {
            const speed = (currentWorkers.construction * BASE_SPEED * 0.4);
            ...
            Object.entries(costs).forEach(([res, amount]) => {
              ...
              nextSiteResources[rKey] = Math.max(0, nextSiteResources[rKey] - (amount as number) * step);
            });
      ...
      setSiteResources(nextSiteResources);
      setProgress(nextProgress);
      if (triggerBuild) handleBuildRef.current(triggerBuild);
  ...
  const handleBuild = useCallback((stage: BuildStage) => {
    const floorIdx = currentFloorIndexRef.current;

    setFloors(prev => {
      const next = [...prev];
      const currentFloor = next[floorIdx];
      if (!currentFloor) return prev;

      next[floorIdx] = {
        ...currentFloor,
        stage: stage,
        isLanded: stage === 'FLOOR' ? (floorIdx === 0 ? true : false) : currentFloor.isLanded
      };

      if (stage === 'WALLS') {
        const nextId = floorIdx + 1;
        if (!next[nextId]) {
          next.push({ id: nextId, stage: 'NONE', isPerfect: false, timestamp: Date.now(), isLanded: false });
        }

        // Use timeout to avoid state update during render/simulation loop sync issues
        setTimeout(() => {
          setCurrentFloorIndex(nextId);
          setMaxHeight((prevMax) => Math.max(prevMax, nextId));
    ...
    if (stage === 'FLOOR' && floorIdx !== 0) {
      setCameraTargetHeight(floorIdx * FLOOR_HEIGHT);
    }
  }, []);
